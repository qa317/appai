...
# ──────────────────────────────────────────────
# 1.  CACHED DATA LOADING  (runs ONCE until refresh)
# ──────────────────────────────────────────────
...
def load_raw_sheet(raw_sheet_id):
...
def load_qa_log(qa_id):
    qasheet = "https://docs.google.com/spreadsheets/d/1V1SfBZUwHN0NtXFIoiXEh7JGkpTUOLZnGAfFN8QVXYQ/export?format=csv&" + qa_id
    return pd.read_csv(qasheet)

@st.cache_data(show_spinner="Loading sampling sheet…")
def load_sampling_sheet(qa_id):
    samplingsheet = "https://docs.google.com/spreadsheets/d/1U0Y7TQnTFEg1edMb0IHejOxv9S2YLY2UH-tp1qzXyBg/export?format=csv&" + qa_id
    return pd.read_csv(samplingsheet)

@st.cache_data(show_spinner="Loading HFC sheet…")
def load_hfc_sheet(qa_id3):
    hfcsheet = "https://docs.google.com/spreadsheets/d/16EWCV7HTEx729ILvsYa72LkJ1P1Sw7Fo2R0FzXs3GvE/export?format=csv&" + qa_id3
    return pd.read_csv(hfcsheet)
...
def compute_vid(row, tool_col_map):
...
<style>
  ...
  .section-hdr {
      ...
      border-left: 4px solid #6366f1; padding-left: 12px; margin: 28px 0 10px;
  }
  ...
  .metric-card .label { font-size: 12px; color: #64748b; font-weight: 600; text-transform: uppercase; letter-spacing: .04em; }
  .metric-card .val   { font-size: 28px; font-weight: 800; color: #0f172a; margin-top: 4px; }

  /* ── Link boxes ── */
  .link-box {
      padding: 14px; border-radius: 14px; border: 1px solid #e2e8f0;
      background: #fff; box-shadow: 0 1px 3px rgba(0,0,0,.04);
      text-align: center; font-size: 14px; transition: all .2s ease;
  }
  .link-box:hover { box-shadow: 0 4px 12px rgba(0,0,0,.08); transform: translateY(-1px); }
  .link-box .lbl { margin-bottom: 6px; font-weight: 600; color: #475569; }
  .link-box a    { color: #6366f1; text-decoration: none; font-weight: 600; }
  .link-box a:hover { text-decoration: underline; }

  .link-box-na {
      padding: 14px; border-radius: 14px; border: 1px dashed #cbd5e1;
      background: #f8fafc; text-align: center; font-size: 14px; color: #94a3b8;
  }
  .link-box-na .lbl { margin-bottom: 6px; font-weight: 600; }
  ...
  /* ── Refresh btn area ── */
  .refresh-row { display: flex; justify-content: flex-end; align-items: center; gap: 8px; margin-bottom: 4px; }
  ...
  .top-bar h1 { margin: 0 !important; font-size: 26px !important; }
</style>
...
if not st.session_state.logged_in:
    ...
    with placeholder.form("login"):
        ...
        st.markdown("""
        <div style="background:#f0f8ff; padding:20px; border-radius:12px; border-left:6px solid #6366f1; margin-top:16px;">
          <h3 style="color:#312e81; margin-bottom:10px;">📘 About This App</h3>
          <p style="color:#1e293b; font-size:15px; line-height:1.6;">
            Welcome to our interactive progress tracker!
            This tool provides real-time data collection insights, QA monitoring,
            and user-friendly summaries to help you stay on top of every project.
          </p>
        </div>
        """, unsafe_allow_html=True)
    ...
    if submit:
        if username in user_dict and password == user_dict[username]["password"]:
            ...
            st.markdown("""
            <div style="background:#ffe6e6; padding:20px; border-left:6px solid #ef4444; border-radius:12px;">
              <h2 style="color:#b91c1c; margin-bottom:10px;">🔒 Access Denied</h2>
              <p style="color:#4d0000; font-size:16px;">
                You've entered an <strong>empty</strong> or <strong>incorrect password</strong>.
...
if st.session_state.logged_in:
    ...
    # ── Title row + Refresh + Logout ──
    title_col, btn_col = st.columns([5, 1])
    with title_col:
        st.markdown("<h1 style='margin-bottom:0;'>📊 QA & DC Progress Tracker</h1>", unsafe_allow_html=True)
    with btn_col:
        rcol1, rcol2 = st.columns(2)
        with rcol1:
            if st.button("🔄 Refresh", use_container_width=True, help="Reload all data from Google Sheets"):
                st.cache_data.clear()
                st.rerun()
        with rcol2:
    ...
    with cols1:
        ...
        main_project = st.selectbox("Select main project", main_project_names, key="selectbox_1", label_visibility="collapsed")
    ...
    # PROJECT TIMELINE
    # ─────────────────────────
    st.markdown('<div class="section-hdr">Project Timeline</div>', unsafe_allow_html=True)
        ...
        for phase, ps_c, pe_c, as_c, ae_c in PHASES:
            ...
            if pd.notna(a_s):
                ...
                fig.add_trace(go.Scatter(
                    ...
                    showlegend=False,
                    hovertemplate=f"<b>{project}</b><br>{phase}<br>Actual: {a_s:%d-%b-%Y} → {actual_end:%d-%b-%Y}"
    ...
    fig.update_xaxes(
        range=[
            project_data[[ps_c for _, ps_c, _, _, _ in PHASES if ps_c in project_data.columns]].min().min(),
    ...
    with collll1:
        st.markdown('<div class="section-hdr">Round / Sub-Project</div>', unsafe_allow_html=True)
        selected_project = st.selectbox("Select round", project_names, key="selectbox_2", label_visibility="collapsed")
    ...
    def generate_stylish_horizontal_roadmap_html(steps, current_step_label):
        ...
        for i, step in enumerate(processed_steps):
            ...
            <div class="step-node flex flex-col items-center text-center {node_class}" style="width: {100 / num_steps}%;">
                ...
                <p class="mt-12 text-sm font-medium {text_class} w-28">{label}</p>
            </div>
            """
    ...
    # PROJECT LINKS
    # ─────────────────────────
    st.markdown('<div class="section-hdr">Project Links</div>', unsafe_allow_html=True)
    ...
    if proj_completed == "Yes":
        ...
        st.markdown("""
        <div style="background:#f0f4f8; border-left:6px solid #6366f1; padding:20px; border-radius:12px;">
          <h3 style="color:#4338ca; margin-top:0;">📁 Project Archived</h3>
          <p style="font-size:16px; color:#334155; line-height:1.6;">
            This project has been <strong>archived</strong>. For complete information on <em>sampling</em> and <em>site visits</em>,
            please refer to the <strong>Document</strong> field. Relevant <strong>datasets</strong> and <strong>trackers</strong> are also available.
          </p>
        </div>
        """, unsafe_allow_html=True)
        st.markdown("<br>", unsafe_allow_html=True)
        ...
        # DATA METRICS
        # ─────────────────────────
        st.markdown('<div class="section-hdr">Data Metrics</div>', unsafe_allow_html=True)
        ...
        t = load_raw_sheet(raw_sheet_id).copy()
        qalog = load_qa_log(Project_QA_ID2).copy()
        ...
        t['V_ID'] = t.apply(lambda row: compute_vid(row, tool_col_map), axis=1)

        tari = load_sampling_sheet(Project_QA_ID).copy()
        ...
        fig_timeline = px.line(dff, x='Date', y='N', title='Timeline of Data')
        fig_timeline.update_traces(mode='lines+markers', marker=dict(color='#6366f1', size=8))
        ...
        def build_map(geo_raw, counts_df):
            ...
            folium.Choropleth(
                geo_data=geo, data=counts_df,
                columns=["Province", "count"], key_on="feature.properties.NAME_1",
                fill_color="YlGnBu", fill_opacity=0.8, line_opacity=0.25,
        ...
        # SAMPLE TRACKING
        # ─────────────────────────
        st.markdown('<div class="section-hdr">Sample Tracking</div>', unsafe_allow_html=True)
        ...
        fig1 = go.Figure(data=[go.Pie(
            labels=labels1, values=values1, hole=0.6,
            textinfo='label+percent',
            marker=dict(colors=['#6366f1', '#e2e8f0'])
        )])
        fig1.update_layout(title_text='Data Collection Progress', title_x=0, margin=dict(t=40, b=10))

        fig2 = go.Figure(data=[go.Pie(
            labels=labels2, values=values2, hole=0.6,
            textinfo='label+percent',
            marker=dict(colors=['#22c55e', '#ef4444', '#f59e0b', '#cbd5e1'][:len(g)])
        )])
        fig2.update_layout(title_text='Data QA Progress', title_x=0, margin=dict(t=40, b=10))
        ...
        t_filtered = t[t['QA_Status'].isin(qastatus)]
        m_df = tari[~tari.V_ID.isin(t_filtered.V_ID)]
        m_df['Type'] = 'Missing Data'
        ext = t_filtered[(~t_filtered.V_ID.isin(tari.V_ID)) & (t_filtered.QA_Status == 'Approved')][['Tool', 'V_ID', 'KEY', 'QA_Status']]
        ext['Type'] = 'Extra data'
        dup = t_filtered[t_filtered.V_ID.duplicated(keep='first')][['Tool', 'V_ID', 'KEY', 'QA_Status']]
        ...
        # SUMMARY GENERATION
        # ─────────────────────────
        st.markdown('<div class="section-hdr">Summary Generation</div>', unsafe_allow_html=True)
        st.info(
        ...
        with col3:
            ...
            if disag2:
                st.markdown("<p style='font-weight:700;color:#0f172a;'>DC Progress Summary:</p>", unsafe_allow_html=True)
        ...
        with col4:
            disag = st.multiselect('Create Dataset Summary:', tall.columns.tolist(), default=def_var1,
                                   help='Create summaries based on selected columns.')
            if disag:
                st.markdown("<p style='font-weight:700;color:#0f172a;'>Summary:</p>", unsafe_allow_html=True)
                if len(disag) == 1:
        ...
        if 'tall2' in locals():
            disag_raw = st.multiselect('Tryouts Summary (Phone Surveys):', tall2.columns.tolist(), def_var2,
                                       help='For phone surveys where multiple attempts may be necessary.')
            if disag_raw:
                st.markdown("<p style='font-weight:700;color:#0f172a;'>Raw Data (Tryouts):</p>", unsafe_allow_html=True)
                if len(disag_raw) == 1:
    ...
    with st.expander(header, expanded=False):
        st.markdown("""
        <style>
          .upd-day{margin:10px 0 14px 0;padding-left:14px;border-left:3px solid #c7d2fe;}
          .upd-date{font-weight:700;margin-bottom:6px;color:#4338ca;}
        ...
        for i, (d, msgs) in enumerate(days):
            st.markdown(
                ...
                <div class="upd-day">
                  ...
                  {''.join([f"<div class='upd-item'><span class='upd-dot'>•</span>{m_txt}</div>" for m_txt in msgs])}
                </div>
    ...
    # ─────────────────────────
    if proj_completed != "Yes":
        sr = st.button("📊 Generate Surveyor Performance Report", key="create_report_btn", use_container_width=False)
        ...
        if sr:
            ...
            hfc = load_hfc_sheet(Project_QA_ID3).copy()
...
<script>
  ...
  function formatComments(raw) {{
    ...
    while ((match = re.exec(s)) !== null) {{
      if (lastDate !== null) {{ blocks.push({{ date:lastDate, body: s.slice(lastIndex, match.index).trim() }}); }}
      lastDate = match[1]; lastIndex = re.lastIndex;
    }}
  ...
  function render() {{
    ...
    for (const i of data) {{
      ...
      if (q) {{
        const blob = ((i.Surveyor_Name||"")+" "+(i.KEY||"")+" "+(i.Site_Visit_ID||"")+" "+(i.QA_Status||"")+" "+(i.Location||"")+" "+(i.Issue_Type||"")+" "+(i.Issue_Description||"")+" "+(i.surveyor_response||"")).toLowerCase();
        if (!blob.includes(q)) continue;
      }}
      out.push(`<tr>
        <td>
          <div class="muted" style="font-weight:900;letter-spacing:.08em;text-transform:uppercase;color:#4f46e5">${{esc(i.Surveyor_Name)}}</div>
          ...
          <div class="muted" style="margin-top:10px"><span style="color:#dc2626;font-weight:900">ISSUE:</span><div class="issue-comments">${{formatComments(i.Issue_Description)}}</div></div>
        </td>
        <td><div class="response-comments">${{ i.surveyor_response? formatComments(i.surveyor_response): '<div class="awaiting-response">Awaiting response from DC/field...</div>'}}</div></td>
        <td class="c"><span class="pill" style="background:#e2e8f0;color:#0f172a">${{esc(i.severity)}}</span></td>
        <td class="c"><span class="pill" style="background:${{i.issue_resolved === "Yes" ? "#dcfce7" : "#ffe4e6"}};color:${{i.issue_resolved === "Yes" ? "#166534" : "#9f1239"}}">${{i.issue_resolved === "Yes" ? "Closed" : "Pending"}}</span></td>
      </tr>`);
    }}
